---
name: super-simple-songs
description: >-
  Ultra-simple singable loop patterns for your teachable topic. Triggers: super simple songs, /super-simple-songs, singable, kids songs. Part of VIDEO IDEAS.
disable-model-invocation: true
---

# Super Simple Songs

Slash: `/super-simple-songs`

Before starting, read [shared-rules.md](../../video-ideas/shared-rules.md).

## Ethos

Ultra-simple singable teaching loops: tiny vocabulary, repetition, call-and-response, routine actions set to melody. Titles name the action or concept + "Kids Songs" / channel tag. Audience contract = predictable structure a child (or beginner) can join immediately.

## Top titles

- This Is The Way | Kids Songs | Super Simple Songs
- Twinkle Twinkle Little Star | Nursery Rhymes for Kids | Super Simple Songs
- This Is The Way We Get Dressed | Kids Songs | Super Simple Songs
- Five Little Ducks | Kids Songs | Super Simple Songs
- If You're Happy And You Know It | Kids Songs | Super Simple Songs
- Put On Your Shoes | Clothing and Routines Song for Kids | Super Simple Songs
- Baby Shark | featuring Finny The Shark | Super Simple Songs
- The Alphabet Is So Much Fun | Kids Songs | Super Simple Songs

## Steps

1. Take user's expertise / experience / topic.
2. Read Top titles. For each (or a strong subset), transpose the *structure* onto the user's world.
3. Ask implicitly: "What would Super Simple Songs do if they focused on [user's thing]?"
4. Output a list of suggested titles (and optional one-line topic notes). Stop. User picks.

## Anti-patterns

- Don't copy their niche content; transpose the title grammar
- Don't expand into scripts or production plans
- Don't skip the Top titles list and improvise "vibes only"
- Don't claim endorsement or impersonate
