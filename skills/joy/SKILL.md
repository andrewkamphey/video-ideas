---
name: joy
description: >-
  Rapid inventory of joy — write it down and move on; don't marinate. Triggers: joy, joyful, over excited, write and move on. Part of VIDEO IDEAS.
disable-model-invocation: true
---

# Joy

Slash: `/joy`

Before starting, read [shared-rules.md](../video-ideas/shared-rules.md).

## Purpose

Opposite of **Bad**: inventory of uplift, not harm — same pace: write it down, keep moving. What are you feeling joyful about, over excited, beyond belief. Could be something in your past that makes you feel joy now — some smell, some taste. Could be something you're hopeful for, something to look forward to. What are you feeling joy RIGHT NOW for. What's gotten you out of your normal mental state — a movie, a line, a joke you remember, a person.

Don't just savor in the memory. Write down the note and move on. The point of JOY is not to marinate in the memory but rather process through the feelings and iterate.

## Inputs

Optional life/work lens; otherwise open.

## Mode

`batch` — rapid list through prompt clusters; facilitator keeps tempo ("next"); no long reminiscence.

## Steps

1. Set tempo: write it down and move on — no marinating.
2. Cluster — peak joy / over-excited / beyond belief: list fast.
3. Cluster — past joy that hits now (smell, taste, sensory): list fast.
4. Cluster — hopeful for / looking forward to: list fast.
5. Cluster — joy RIGHT NOW: list fast.
6. Cluster — what broke normal mental state (movie, line, joke, person): list fast.
7. Optional light pass (only after dump): video idea sparks — not during the list.

## Stop

User has a punchy joy inventory. Optional video mapping only if requested.

## Output

Flat punchy list (short notes). Optional light pass to video ideas after the dump — not during.

## Anti-patterns

- Do not marinate, narrate long stories, or "savor" mid-list
- Do not turn into Grateful's structured who/why analysis unless asked
- Do not force every item into a video title during the dump
- Do not soften into mild "nice things" — allow over-excited / beyond belief
