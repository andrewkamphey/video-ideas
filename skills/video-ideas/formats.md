# Formats

Repeatable video molds you run over and over. Same skeleton, new topic each upload.

**Format ≠ series.** Do not announce "Season 1" or "this is my new series." You work the mold forever.

## Entry shape

```yaml
id: x-hacks-you-should-know
name: X Hacks You Should Know
skeleton: "[Tool/Topic] hack you should know"
fill: specific app, feature, or niche (Google Sheets, Photoshop, …)
platforms: [youtube, tiktok, reels, shorts]
notes: One hack per video. Demo in-body. Title = promise of the hack.
```

## How tools use formats

- After a brainstorm dump (BIG, PAY, Iterate, Hooks, etc.), optional: "Pick a format from `formats.md` and generate 10 fills."
- User can say "run Iterate inside X Hack You Should Know."
- Adding a format = new entry below — no SKILL rewrite.

---

## Seed formats

### X Hack You Should Know

```yaml
id: x-hack-you-should-know
name: X Hack You Should Know
skeleton: "[Tool] hack you should know"
fill: specific app, feature, or niche
platforms: [youtube, tiktok, reels, shorts]
notes: One hack per video. Demo in-body. Title = promise of the hack.
```

### I Was Today Years Old

```yaml
id: i-was-today-years-old
name: I Was Today Years Old
skeleton: "I was today years old when I learned [one thing]"
fill: one discovery in your domain
platforms: [youtube, tiktok, reels, shorts]
notes: Shock + demo. One discovery only.
```

### Things I Wish I Knew

```yaml
id: things-i-wish-i-knew
name: Things I Wish I Knew
skeleton: "Things I wish I knew before [doing X]"
fill: one lesson per video in a domain
platforms: [youtube, tiktok]
notes: Not a dump list — one lesson, one video.
```

### Stop Doing X

```yaml
id: stop-doing-x
name: Stop Doing X
skeleton: "Stop doing [bad habit]. Do this instead"
fill: one bad habit + replacement
platforms: [youtube, tiktok, reels]
notes: Name the habit bluntly; show the fix.
```

### X vs Y

```yaml
id: x-vs-y
name: X vs Y
skeleton: "[X] vs [Y] Which should you use?"
fill: one pairwise comparison
platforms: [youtube, tiktok]
notes: Pick a winner or "use when" — one pair only.
```

### Only Use This If…

```yaml
id: only-use-this-if
name: Only Use This If…
skeleton: "Only use [tool/method] if [exact condition]"
fill: one tool + the condition that makes it right
platforms: [youtube, tiktok]
notes: The condition is the hook.
```

### FREE vs $10 vs $100

```yaml
id: tiered-options
name: FREE vs $10 vs $100
skeleton: "FREE vs $10 vs $100 [outcome]"
fill: tiered options for the same outcome
platforms: [youtube, tiktok]
notes: Same outcome, three price points.
```

### What I'd Do Differently

```yaml
id: what-id-do-differently
name: What I'd Do Differently
skeleton: "What I'd do differently on [past project]"
fill: one past project; one change
platforms: [youtube]
notes: One project, one lesson — not a career retrospective.
```

### I Made X in Y

```yaml
id: I-made
name: I made X in Y
skeleton: "I made X in [topic/thing]"
fill: one thing I made, that is simlar to another thing.
platforms: [youtube, tiktok]
notes: Actually show the thing you made.
```

### Green → Red Flags

```yaml
id: green-red-flags
name: Green → Red Flags
skeleton: "Green flags / red flags for [X]"
fill: one green, one red — or one flag per video
platforms: [youtube, tiktok]
notes: Pick one density and keep it.
```

### Tools I Actually Use

```yaml
id: tools-i-actually-use
name: Tools I Actually Use
skeleton: "The [tool] I actually use for [job]"
fill: one tool per video
platforms: [youtube, tiktok]
notes: Not a kit dump — one tool, one reason.
```

### Reply to Comments

```yaml
id: reply-to-comments
name: Reply to Comments
skeleton: "You asked: [comment]"
fill: one comment = one video
platforms: [youtube, tiktok]
notes: Quote the comment; answer fully.
```