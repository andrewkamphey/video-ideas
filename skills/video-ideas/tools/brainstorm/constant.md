...
`batch`. List the constants; then optionally turn each into a video angle.

## Steps

1. Ask for domain / life & work context. What do outsiders assume is easy or done for you now?
2. List recurring work that never stops. Specific, ongoing, not one-off projects.
3. Name the invisible load: mental weight, maintenance, re-decisions.
4. Optional: title/idea per constant for people who don't see that load yet.

## Stop

Constants list captured. User picks what to film.

## Output

1. List of constant weights / recurring work (specific, ongoing)
